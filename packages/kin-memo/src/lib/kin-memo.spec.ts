...
describe('KinMemo', () => {
  ...
  it('invalid', () => {
    ...
    expect(() => KinMemo.new(8, TransactionType.Earn, 1, fk)).toThrow(
      'invalid version'
    );
    ...
    // invalid tx type
    expect(() => KinMemo.new(1, TransactionType.Unknown, 1, fk)).toThrow(
      'cannot use unknown transaction type'
    );
  ...
  it('memo from', () => {
    const valid = KinMemo.from(
      Buffer.from('KQQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=', 'base64')
    );
    expect(KinMemo.isValid(valid)).toBeTruthy();
    expect(KinMemo.isValid(valid, true)).toBeFalsy();

    const strictlyValid = KinMemo.from(
      Buffer.from('JQQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=', 'base64')
    );
    ...
    const unknownTxType = KinMemo.from(
      Buffer.from('RQUAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=', 'base64')
    );
